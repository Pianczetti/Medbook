...
final class AddonGridDefinitionFactory extends AbstractGridDefinitionFactory
{
    public const GRID_ID = 'medbook_addon';
    ...
    {
        return $this->trans('Add-ons', [], 'Modules.Medbookbooking.Admin');
    }
    ...
    protected function getColumns(): ColumnCollection
    {
        return (new ColumnCollection())
            ->add((new BulkActionColumn('bulk'))
                ->setOptions(['bulk_field' => 'id_addon'])
            )
            ->add((new DataColumn('id_addon'))
                ->setName($this->trans('ID', [], 'Admin.Global'))
                ->setOptions(['field' => 'id_addon'])
            )
            ...
            ->add((new DataColumn('price'))
                ...
                ->setOptions(['field' => 'price'])
            )
            ->add((new ToggleColumn('is_active'))
                ->setName($this->trans('Active', [], 'Admin.Global'))
                ->setOptions([
                    'field' => 'is_active',
                    'primary_field' => 'id_addon',
                    'route' => 'admin_medbook_booking_addon_toggle_active',
                    'route_param_name' => 'addonId',
                ])
            )
                ...
                ->setOptions([
                    'actions' => (new RowActionCollection())
                        ->add((new LinkRowAction('edit'))
                            ->setIcon('edit')
                            ->setOptions([
                                'route' => 'admin_medbook_booking_addon_edit',
                                'route_param_name' => 'addonId',
                                'route_param_field' => 'id_addon',
                            ])
                        )
                        ->add((new LinkRowAction('delete'))
                            ->setIcon('delete')
                            ->setOptions([
                                'route' => 'admin_medbook_booking_addon_delete',
                                'route_param_name' => 'addonId',
                                'route_param_field' => 'id_addon',
                            ])
                        ),
    ...
    protected function getFilters(): FilterCollection
    {
        return (new FilterCollection())
            ->add(
                (new Filter('id_addon', TextType::class))
                    ->setTypeOptions(['required' => false])
                    ->setAssociatedColumn('id_addon')
            )
            ->add(
            ...
            ->add(
                (new Filter('actions', SearchAndResetType::class))
                    ->setTypeOptions([
                        ...
                        'redirect_route' => 'admin_medbook_booking_addon_index',
                    ])
